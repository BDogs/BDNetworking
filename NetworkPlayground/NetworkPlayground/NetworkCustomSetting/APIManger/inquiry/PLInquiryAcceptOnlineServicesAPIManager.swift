...
enum PLInquiryAcceptOnlineServicesAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquiryAcceptOnlineServicesAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLInquiryAcceptOnlineServicesAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/accept/onlineServices"
    }
}

extension PLInquiryAcceptOnlineServicesAPIManager: BDAPIManagerValidator {
