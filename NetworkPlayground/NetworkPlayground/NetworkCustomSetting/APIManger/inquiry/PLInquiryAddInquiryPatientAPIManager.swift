...
enum PLInquiryAddInquiryPatientAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquiryAddInquiryPatientAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLInquiryAddInquiryPatientAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/addInquiryPatient"
    }
}

extension PLInquiryAddInquiryPatientAPIManager: BDAPIManagerValidator {
