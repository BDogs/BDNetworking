...
enum PLInquirySendPrescriptionAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquirySendPrescriptionAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLInquirySendPrescriptionAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/sendPrescription"
    }
}

extension PLInquirySendPrescriptionAPIManager: BDAPIManagerValidator {
