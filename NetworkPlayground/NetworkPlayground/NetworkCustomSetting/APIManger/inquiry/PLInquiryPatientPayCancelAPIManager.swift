...
enum PLInquiryPatientPayCancelAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquiryPatientPayCancelAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLInquiryPatientPayCancelAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/patient/payCancel"
    }
}

extension PLInquiryPatientPayCancelAPIManager: BDAPIManagerValidator {
