...
enum PLInquiryGetPrescriptionDetailAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquiryGetPrescriptionDetailAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLInquiryGetPrescriptionDetailAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/getPrescriptionDetail"
    }
}

extension PLInquiryGetPrescriptionDetailAPIManager: BDAPIManagerValidator {
