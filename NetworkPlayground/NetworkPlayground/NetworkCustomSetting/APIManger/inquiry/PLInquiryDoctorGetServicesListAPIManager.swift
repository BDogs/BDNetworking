...
enum PLInquiryDoctorGetServicesListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquiryDoctorGetServicesListAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLInquiryDoctorGetServicesListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/doctor/getServicesList"
    }
}

extension PLInquiryDoctorGetServicesListAPIManager: BDAPIManagerValidator {
