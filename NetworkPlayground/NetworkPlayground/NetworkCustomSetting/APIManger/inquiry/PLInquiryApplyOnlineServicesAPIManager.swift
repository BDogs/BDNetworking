...
enum PLInquiryApplyOnlineServicesAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquiryApplyOnlineServicesAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLInquiryApplyOnlineServicesAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/apply/onlineServices"
    }
}

extension PLInquiryApplyOnlineServicesAPIManager: BDAPIManagerValidator {
