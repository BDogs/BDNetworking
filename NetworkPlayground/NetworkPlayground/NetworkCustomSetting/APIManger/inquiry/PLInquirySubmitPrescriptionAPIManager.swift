...
enum PLInquirySubmitPrescriptionAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquirySubmitPrescriptionAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLInquirySubmitPrescriptionAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/submitPrescription"
    }
}

extension PLInquirySubmitPrescriptionAPIManager: BDAPIManagerValidator {
