...
enum PLInquiryGetIntroduceDetailAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquiryGetIntroduceDetailAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLInquiryGetIntroduceDetailAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/getIntroduceDetail"
    }
}

extension PLInquiryGetIntroduceDetailAPIManager: BDAPIManagerValidator {
