...
enum PLInquiryRefuseOnlineServicesAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquiryRefuseOnlineServicesAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLInquiryRefuseOnlineServicesAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/refuse/onlineServices"
    }
}

extension PLInquiryRefuseOnlineServicesAPIManager: BDAPIManagerValidator {
