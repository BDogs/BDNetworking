...
enum PLInquiryPatientFinishInquiryAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquiryPatientFinishInquiryAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLInquiryPatientFinishInquiryAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/patient/finishInquiry"
    }
}

extension PLInquiryPatientFinishInquiryAPIManager: BDAPIManagerValidator {
