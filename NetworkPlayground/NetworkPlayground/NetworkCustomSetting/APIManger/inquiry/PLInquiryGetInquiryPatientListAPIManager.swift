...
enum PLInquiryGetInquiryPatientListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInquiryGetInquiryPatientListAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLInquiryGetInquiryPatientListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquiry/getInquiryPatientList"
    }
}

extension PLInquiryGetInquiryPatientListAPIManager: BDAPIManagerValidator {
