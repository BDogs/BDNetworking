...
enum PLFollowupGetDiaryListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLFollowupGetDiaryListAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLFollowupGetDiaryListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "followup/getDiaryList"
    }
}

extension PLFollowupGetDiaryListAPIManager: BDAPIManagerValidator {
