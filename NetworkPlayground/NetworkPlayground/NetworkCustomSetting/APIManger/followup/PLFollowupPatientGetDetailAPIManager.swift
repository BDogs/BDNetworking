...
enum PLFollowupPatientGetDetailAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLFollowupPatientGetDetailAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLFollowupPatientGetDetailAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "followup/patient/getDetail"
    }
}

extension PLFollowupPatientGetDetailAPIManager: BDAPIManagerValidator {
