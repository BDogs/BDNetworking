...
enum PLFollowupGetQuestionListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLFollowupGetQuestionListAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLFollowupGetQuestionListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "followup/getQuestionList"
    }
}

extension PLFollowupGetQuestionListAPIManager: BDAPIManagerValidator {
