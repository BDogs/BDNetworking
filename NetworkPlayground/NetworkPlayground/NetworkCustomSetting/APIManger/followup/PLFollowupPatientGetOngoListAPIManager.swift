...
enum PLFollowupPatientGetOngoListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLFollowupPatientGetOngoListAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLFollowupPatientGetOngoListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "followup/patient/getOngoList"
    }
}

extension PLFollowupPatientGetOngoListAPIManager: BDAPIManagerValidator {
