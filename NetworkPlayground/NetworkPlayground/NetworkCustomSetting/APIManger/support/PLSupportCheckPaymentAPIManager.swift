...
enum PLSupportCheckPaymentAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLSupportCheckPaymentAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLSupportCheckPaymentAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "support/checkPayment"
    }
}

extension PLSupportCheckPaymentAPIManager: BDAPIManagerValidator {
