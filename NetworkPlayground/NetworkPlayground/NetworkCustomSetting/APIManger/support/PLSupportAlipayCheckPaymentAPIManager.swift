...
enum PLSupportAlipayCheckPaymentAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLSupportAlipayCheckPaymentAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLSupportAlipayCheckPaymentAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "support/alipay/checkPayment"
    }
}

extension PLSupportAlipayCheckPaymentAPIManager: BDAPIManagerValidator {
