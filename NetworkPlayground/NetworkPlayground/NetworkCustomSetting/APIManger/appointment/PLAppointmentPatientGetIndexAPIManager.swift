...
enum PLAppointmentPatientGetIndexAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLAppointmentPatientGetIndexAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLAppointmentPatientGetIndexAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "appointment/patient/getIndex"
    }
}

extension PLAppointmentPatientGetIndexAPIManager: BDAPIManagerValidator {
