...
enum PLAppointmentDoctorGetAllListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLAppointmentDoctorGetAllListAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLAppointmentDoctorGetAllListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "appointment/doctor/getAllList"
    }
}

extension PLAppointmentDoctorGetAllListAPIManager: BDAPIManagerValidator {
