...
enum PLAppointmentGetDoctorOngoingByDateAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLAppointmentGetDoctorOngoingByDateAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLAppointmentGetDoctorOngoingByDateAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "appointment/getDoctorOngoingByDate"
    }
}

extension PLAppointmentGetDoctorOngoingByDateAPIManager: BDAPIManagerValidator {
