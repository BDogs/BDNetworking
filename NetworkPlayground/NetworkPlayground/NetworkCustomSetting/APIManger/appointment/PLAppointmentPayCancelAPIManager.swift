...
enum PLAppointmentPayCancelAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLAppointmentPayCancelAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLAppointmentPayCancelAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "appointment/payCancel"
    }
}

extension PLAppointmentPayCancelAPIManager: BDAPIManagerValidator {
