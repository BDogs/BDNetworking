...
enum PLPatientGetPatientDetailAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLPatientGetPatientDetailAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLPatientGetPatientDetailAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "patient/getPatientDetail"
    }
}

extension PLPatientGetPatientDetailAPIManager: BDAPIManagerValidator {
