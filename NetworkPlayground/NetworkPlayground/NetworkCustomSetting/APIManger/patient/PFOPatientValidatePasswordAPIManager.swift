...
enum PFOPatientValidatePasswordAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PFOPatientValidatePasswordAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PFOPatientValidatePasswordAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "patient/validate/password"
    }
}

extension PFOPatientValidatePasswordAPIManager: BDAPIManagerValidator {
