...
enum PLDoctorCreateParkinsondbPatientAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLDoctorCreateParkinsondbPatientAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLDoctorCreateParkinsondbPatientAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "doctor/createParkinsondbPatient"
    }
}

extension PLDoctorCreateParkinsondbPatientAPIManager: BDAPIManagerValidator {
