...
enum PLDoctorGetPatientListByTypeAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLDoctorGetPatientListByTypeAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLDoctorGetPatientListByTypeAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "doctor/getPatientListByType"
    }
}

extension PLDoctorGetPatientListByTypeAPIManager: BDAPIManagerValidator {
