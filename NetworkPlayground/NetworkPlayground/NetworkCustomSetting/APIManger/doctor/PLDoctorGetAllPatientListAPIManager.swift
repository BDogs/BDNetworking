...
enum PLDoctorGetAllPatientListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLDoctorGetAllPatientListAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLDoctorGetAllPatientListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "doctor/getAllPatientList"
    }
}

extension PLDoctorGetAllPatientListAPIManager: BDAPIManagerValidator {
