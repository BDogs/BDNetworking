...
enum PLVideoDoctorGetOngoingListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoDoctorGetOngoingListAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoDoctorGetOngoingListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/doctor/getOngoingList"
    }
}

extension PLVideoDoctorGetOngoingListAPIManager: BDAPIManagerValidator {
