...
enum PLVideoDoctorGetServiceListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoDoctorGetServiceListAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoDoctorGetServiceListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/doctor/getServiceList"
    }
}

extension PLVideoDoctorGetServiceListAPIManager: BDAPIManagerValidator {
