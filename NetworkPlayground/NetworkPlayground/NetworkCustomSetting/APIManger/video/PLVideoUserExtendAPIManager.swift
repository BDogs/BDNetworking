...
enum PLVideoUserExtendAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoUserExtendAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoUserExtendAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/user/extend"
    }
}

extension PLVideoUserExtendAPIManager: BDAPIManagerValidator {
