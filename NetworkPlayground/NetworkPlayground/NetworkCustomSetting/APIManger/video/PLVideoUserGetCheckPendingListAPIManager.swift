...
enum PLVideoUserGetCheckPendingListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoUserGetCheckPendingListAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoUserGetCheckPendingListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/user/getCheckPendingList"
    }
}

extension PLVideoUserGetCheckPendingListAPIManager: BDAPIManagerValidator {
