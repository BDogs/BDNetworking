...
enum PLVideoDoctorGetCheckedListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoDoctorGetCheckedListAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoDoctorGetCheckedListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/doctor/getCheckedList"
    }
}

extension PLVideoDoctorGetCheckedListAPIManager: BDAPIManagerValidator {
