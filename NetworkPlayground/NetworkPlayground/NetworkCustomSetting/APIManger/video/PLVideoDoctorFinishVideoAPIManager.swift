...
enum PLVideoDoctorFinishVideoAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoDoctorFinishVideoAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoDoctorFinishVideoAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/doctor/finishVideo"
    }
}

extension PLVideoDoctorFinishVideoAPIManager: BDAPIManagerValidator {
