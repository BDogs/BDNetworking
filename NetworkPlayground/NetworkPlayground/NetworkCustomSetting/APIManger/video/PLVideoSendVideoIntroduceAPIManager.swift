...
enum PLVideoSendVideoIntroduceAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoSendVideoIntroduceAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoSendVideoIntroduceAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/sendVideoIntroduce"
    }
}

extension PLVideoSendVideoIntroduceAPIManager: BDAPIManagerValidator {
