...
enum PLVideoUserAddVideoTimeAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoUserAddVideoTimeAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoUserAddVideoTimeAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/user/addVideoTime"
    }
}

extension PLVideoUserAddVideoTimeAPIManager: BDAPIManagerValidator {
