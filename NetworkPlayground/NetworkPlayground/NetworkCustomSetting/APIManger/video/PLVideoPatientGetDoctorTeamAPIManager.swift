...
enum PLVideoPatientGetDoctorTeamAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoPatientGetDoctorTeamAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoPatientGetDoctorTeamAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/patient/getDoctorTeam"
    }
}

extension PLVideoPatientGetDoctorTeamAPIManager: BDAPIManagerValidator {
