...
enum PLVideoUserConfirmAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoUserConfirmAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoUserConfirmAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/user/confirm"
    }
}

extension PLVideoUserConfirmAPIManager: BDAPIManagerValidator {
