...
enum PLVideoDoctorStartVideoAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoDoctorStartVideoAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoDoctorStartVideoAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/doctor/startVideo"
    }
}

extension PLVideoDoctorStartVideoAPIManager: BDAPIManagerValidator {
