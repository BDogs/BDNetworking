...
enum PLVideoAddCommentAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoAddCommentAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoAddCommentAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/addComment"
    }
}

extension PLVideoAddCommentAPIManager: BDAPIManagerValidator {
