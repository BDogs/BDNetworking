...
enum PLVideoDoctorGetAllListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLVideoDoctorGetAllListAPIManager: BDAPIBaseManager {
    // MARK: - override
    required init() {
        super.init()
        self.validator = self
...
extension PLVideoDoctorGetAllListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "video/doctor/getAllList"
    }
}

extension PLVideoDoctorGetAllListAPIManager: BDAPIManagerValidator {
