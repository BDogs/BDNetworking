...
enum PLInqueryDoctorGetFinishListAPIManagerResponseError: Int, Error, LocalizedError {
    case `default` = 0
...
class PLInqueryDoctorGetFinishListAPIManager: BDAPIBaseManager {
    // MARK: - override
    override init() {
        super.init()
        self.validator = self
...
extension PLInqueryDoctorGetFinishListAPIManager: BDAPIManagerProtocal {
    ...
    func relativeUrl() -> String {
        return "inquery/doctor/getFinishList"
    }
}

extension PLInqueryDoctorGetFinishListAPIManager: BDAPIManagerValidator {
